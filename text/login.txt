...
router.post('/login', async (req, res) => {
    const q = req.body;
    const sql = "SELECT ps.person_username, "
    ...
    const params = [key, q.inputUsername, q.inputPassword]
    con.query(sql, params, (err,rows) => {
        if (err) {
            //console.log(err);
            return res.json({status: '0', message: 'ไม่สามารถเชื่อมต่อฐานข้อมูลบุคลากรได้'})
        } else {
            //console.log(rows[0].person_id.toString('utf8'));
            if (rows.length > 0) {
                fn.dataLog('LOGIN', '', sql, q.inputUsername);
                //console.log(ip.getClientIp(req));
                return res.json({person_username: rows[0].person_username, 
                ...
                return res.json({status: '0', message: 'Username หรือ Password ไม่ถูกต้อง<br>หรือคุณไม่มีสิทธิ์เข้าใช้งาน!'})
            }
        }
    });  
});
...
router.post('/level', async (req, res) => {
    ...
    con.query(sql, params, (err,rows) => {
        if (err) {
            //console.log(err);
            return res.json({status: '0', message: 'ไม่สามารถเชื่อมต่อฐานข้อมูลระดับปฏิบัติงานได้'})
        } else {
            if (rows.length > 0) {
                //console.log(rows);
                return res.json(rows)
            } else {
                return res.json({status: '0', message: 'ไม่พบข้อมูลหน่วยงานที่สังกัด'})
            }
        }
    }); 
});
...
router.post('/right', async (req, res) => {
    ...
    con.query(sql, params, (err,rows) => {
        if (err) {
            //console.log(err);
            return res.json({status: "0", message: "ไม่สามารถเชื่อมต่อฐานข้อมูลสิทธิ์การใช้งานได้"})
        } else {
            if (rows.length > 0) {
                //console.log(rows);
